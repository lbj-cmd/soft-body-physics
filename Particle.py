...
class Particle:
    ...
    position     = Vector(0.0, 0.0, 0.0)# current position
    previous     = Vector(0.0, 0.0, 0.0)# previous time-step [t-dt] position
    velocity     = Vector(0.0, 0.0, 0.0)# [readme] particle velocity
    acceleration = Vector(0.0, 0.0, 0.0)# acceleration of the particle
    ...
    # @param    z           particle depth position relative to the world
    # @param    material    specify this particle's material; default if none provided
    # @return   null
    #
    def __init__(self, world, x=0.0, y=0.0, z=0.0, material=None):
        self.world    = world
    ...
    def Restrain(self):
        ...
        if self.position.z < 0.0:
            ...
            self.previous.z = self.position.z + self.material.bounce * distance.z
            #
            j = distance.x
            k = distance.z * self.material.friction
            t = j
            if j != 0.0:
                t /= abs(j)
            if abs(j) <= abs(k):
        ...
        elif self.position.z > self.world.size.z:
            distance = self.position - self.previous
            self.position.z = 2.0 * self.world.size.z - self.position.z
            self.previous.z = self.position.z + self.material.bounce * distance.z
            #
            j = distance.x
            k = distance.z * self.material.friction
