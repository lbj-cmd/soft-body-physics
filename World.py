...
class World:
    #
    size        = Vector(0.0, 0.0, 0.0)  # world size/boundaries
    hsize       = Vector(0.0, 0.0, 0.0)  # half-size world size/boundaries
    gravity     = Vector(0.0, 0.0, 0.0)  # global gravitational acceleration
    step        = 0                 # time step
    ...
    # @param    z           depth position of the particle
    # @return   Particle    object reference of the new particle
    #
    def AddParticle(self, x, y, z=0.0, mat=None):
        particle = Particle(self, x, y, z, mat)
